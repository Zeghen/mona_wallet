...
class TransactionMoneyController extends GetxController implements GetxService {
  ...
  bool get getContactPermissionStatus => authRepo.sharedPreferences.getString(AppConstants.CONTACT_PERMISSION) != PermissionStatus.granted.name;
  ...
  Future<void> fetchContact() async {
    ...
    update();
    if(getContactPermissionStatus){
      return Get.dialog(
        CustomDialog(
          description: '${AppConstants.APP_NAME} use the information you provide including the contact list of the user’s device for various purposes. The contact list is not being stored in database and only used for the purpose of performing any instruction given by the user.',
          icon: Icons.question_mark,
          onTapFalse:() {
            _contactIsLoading = false;
            Get.back();
          } ,
          onTapTrueText: 'Accept',
          onTapFalseText: 'Denied',
          onTapTrue: () {
            Get.back();
            _contactData();
          } ,
  ...
  Future<void> _contactData() async {
    ...
    authRepo.sharedPreferences.setString(AppConstants.CONTACT_PERMISSION, permissionStatus.name);
    ...
    for(int i= 0; i < contacts.length; i++){
      if(contacts[i].phones != null && contacts[i].phones.isNotEmpty
          && contacts[i].displayName != null && contacts[i].displayName.isNotEmpty) {
