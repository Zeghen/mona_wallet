...
class TransactionMoneyBalanceInput extends StatefulWidget {
  ...
  final String countryCode;
   TransactionMoneyBalanceInput({Key key, this.transactionType ,this.contactModel, this.countryCode}) : super(key: key);
  @override
...
class _TransactionMoneyBalanceInputState extends State<TransactionMoneyBalanceInput> {
                    ...
                    if(_inputAmountController.text.isEmpty){
                      ...
                      if(amount <= 0) {
                        ...
                        if(_isCheck && widget.transactionType == TransactionType.SEND_MONEY) {
                          _inSufficientBalance = PriceConverter.balanceWithCharge(amount, splashController.configModel.sendMoneyChargeFlat, false) > profileController.userInfo.balance;
                        }else if(_isCheck && widget.transactionType == TransactionType.CASH_OUT) {
                          _inSufficientBalance = PriceConverter.balanceWithCharge(amount, splashController.configModel.cashOutChargePercent, true) > profileController.userInfo.balance;
                        }else if(_isCheck && widget.transactionType == TransactionType.WITHDRAW_REQUEST) {
                          _inSufficientBalance = PriceConverter.balanceWithCharge(amount, splashController.configModel.withdrawChargePercent, true) > profileController.userInfo.balance;
                        }else if(_isCheck){
  ...
  void _confirmationRoute(double amount) {
    ...
    if(widget.transactionType == 'add_money'){
      ...
      firstWhereOrNull((_method) => _selectedMethodId == _method.id.toString());
      ...
      if(_withdrawMethod != null) {
        ...
        _message = 'please_select_a_method'.tr;
      }
      ...
      Get.to(()=> TransactionMoneyConfirmation(
        inputBalance: amount,
        transactionType:widget.transactionType,
        purpose: widget.transactionType == TransactionType.REQUEST_MONEY ?
        '${TransactionType.REQUEST_MONEY}'.tr : transactionMoneyController.purposeList.isEmpty
            ? Purpose().title
